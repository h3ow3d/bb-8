...
from app.llm.ollama_client import OllamaError, OllamaUnavailableError, chat
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info("k8s-sidekick starting up")
    yield
    logger.info("k8s-sidekick shutting down")
...
async def cluster_review(
    ...
    try:
        ...
        try:
            ...
            llm_explanation = f"LLM error: {exc}"
        ...
        audit_id = write_audit(
            ...
            resources_summary=f"nodes={len(snapshot.nodes)} pods={len(snapshot.pods)} namespaces={len(snapshot.namespaces)}",
...
async def ask(
    ...
    try:
        ...
        logger.warning("Ollama unavailable for /ask: %s", exc)
        answer = f"LLM unavailable: {exc}"
        audit_status = "llm_unavailable"
        error_summary = str(exc)
    except OllamaError as exc:
        logger.warning("Ollama error for /ask: %s", exc)
